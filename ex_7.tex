...
\begin{frame}{Exercise 1 (c) (ii)}
	\textbf{Question (c) (ii)}: How could you adapt the penalty term such that the solution to the projected problem is equivalent to the original Lasso regression? In this case, can we still solve for parameters independently?
		...
		\begin{itemize}
			\item<2-> We can choose $||\Amat \thetab||_1$ as regularization term. This will yeild $\thetab^* = \Amat^T \thetab_{\text{Lasso}}$. It is equivalent to the original Lasso regression in the sense that \textbf{the optimal solution leads to the same risk}. 
...
\begin{frame}{Exercise 1 (c) (iii)}
	...
	\begin{itemize}
		...
		\item<4-> But this does not imply that the solution in the original coordinates $\Amat \thetab^*$ will be sparse.
	\end{itemize}
\end{frame}
